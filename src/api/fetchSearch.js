export const fetchSearch = async ({ queryKey }) => {
    ...
    const queryString = `?q=${queryKey[1].query}&type=${queryKey[1].filter === "all" ? "album,artist,playlist,track,show,episode,audiobook" : queryKey[1].filter}${queryKey[1].limit ? `&limit=${queryKey[1].limit}` : ""}`;
